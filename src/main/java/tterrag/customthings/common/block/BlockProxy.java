...
public class BlockProxy<T extends Block & IBlockCustom> implements IBlockCustom {
    ...
    public boolean canHarvestBlock(EntityPlayer player, int meta) {
        ...
        return held.getItem()
            .getHarvestLevel(held, getHarvestTool(meta)) >= harvestLevel && held.getItem()
                .getToolClasses(held)
                .contains(type.toolType);
    }
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        BlockType type = getType(metadata);
        return type == null || type.drops.length == 0 ? Lists.newArrayList(new ItemStack(block, 1, metadata))
            : type.getStackDrops();
    }

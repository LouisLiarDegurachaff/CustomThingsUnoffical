...
public class AchievementHandler {
    ...
    public void onAchievement(final AchievementEvent event) {
        if (ignoreAchievement || event.entityPlayer.worldObj.isRemote
            || ((EntityPlayerMP) event.entityPlayer).func_147099_x()
        ...
        for (AchievementPage p : AchievementPage.getAchievementPages()) {
            if (p.getAchievements()
                .contains(event.achievement)) {
                page = p;
            }
        }

        final AchievementPage pf = page;
        EnderCore.proxy.getScheduler()
            .schedule(50, new Runnable() {
                ...
                public void run() {
                    ...
                    triggerAchievement(
                        AchievementSource.ACHIEVEMENT_PAGE,
                        event.entityPlayer,
                        pf,
                        event.entityPlayer,
                        event.achievement);
            ...
            for (AchievementType type : AchievementType.lookup.get(source)) {
                if (source.matchesObject(type.sourceObj, in)) {
                    if (type.rewardStack != null) {
                        ...
                        if (!file.hasAchievementUnlocked(type.achievement)
                            && file.canUnlockAchievement(type.achievement)) {
                            ItemStack stack = type.rewardStack.copy();
                            if (type.rewardNBT != null) {
                                stack = ItemUtil.parseStringIntoItemStack(
                                    ItemUtil.getStringForItemStack(stack, true, true) + "$"
                                        + AchievementType.Template
                                            .format(type.rewardNBT, new AchievementData(type.achievement, player)));
                            }

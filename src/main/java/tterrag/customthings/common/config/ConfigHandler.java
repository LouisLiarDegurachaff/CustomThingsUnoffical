...
public class ConfigHandler {
    ...
    public static void preInit(FMLPreInitializationEvent event) {
        ...
        armorReader = new JsonConfigReader<ArmorType>(
            token,
            baseDir.getAbsolutePath() + "/" + "customArmors.json",
            ArmorType.class);
        toolReader = new JsonConfigReader<ToolType>(
            token,
            baseDir.getAbsolutePath() + "/" + "customTools.json",
            ToolType.class);
        itemReader = new JsonConfigReader<ItemType>(
            token,
            baseDir.getAbsolutePath() + "/" + "customItems.json",
            ItemType.class);
        recordReader = new JsonConfigReader<RecordType>(
            token,
            baseDir.getAbsolutePath() + "/" + "customRecords.json",
            RecordType.class);
        blockReader = new JsonConfigReader<BlockType>(
            token,
            baseDir.getAbsolutePath() + "/" + "customBlocks.json",
            BlockType.class);
        // fluidReader = new JsonConfigReader<FluidType>(token, baseDir.getAbsolutePath() + "/" + "customFluids.json",
        // FluidType.class);

        shapedReader = new JsonConfigReader<ShapedJsonRecipe>(
            token,
            baseDir.getAbsolutePath() + "/" + "shapedRecipes.json",
            ShapedJsonRecipe.class);
        shapelessReader = new JsonConfigReader<ShapelessJsonRecipe>(
            token,
            baseDir.getAbsolutePath() + "/" + "shapelessRecipes.json",
            ShapelessJsonRecipe.class);
        smeltingReader = new JsonConfigReader<SmeltingJsonRecipe>(
            token,
            baseDir.getAbsolutePath() + "/" + "smeltingRecipes.json",
            SmeltingJsonRecipe.class);

        achievementReader = new JsonConfigReader<AchievementType>(
            token,
            baseDir.getAbsolutePath() + "/" + "customAchievements.json",
        ...
        if (CompatUtil.isDifficultyRecipesLoaded()) {
            ...
            shapedDifficultyReader = new JsonConfigReader<ShapedJsonDifficultyRecipe>(
                token,
                baseDir.getAbsolutePath() + "/difficultyrecipes/shaped.json",
                ShapedJsonDifficultyRecipe.class);
            shapedDifficultyReader.getBuilder()
                .registerTypeAdapter(
                    new TypeToken<EnumMap<Difficulty, ShapedJsonRecipe>>() {}.getType(),
                    creatorShaped);
            ...
            shapelessDifficultyReader = new JsonConfigReader<ShapelessJsonDifficultyRecipe>(
                token,
                baseDir.getAbsolutePath() + "/difficultyrecipes/shapeless.json",
                ShapelessJsonDifficultyRecipe.class);
            shapelessDifficultyReader.getBuilder()
                .registerTypeAdapter(
                    new TypeToken<EnumMap<Difficulty, ShapelessJsonRecipe>>() {}.getType(),
                    creatorShapeless);
    ...
    public static void assembleResourcePack() {
        ...
        assembler = new ResourcePackAssembler(
            new File(baseDir.getAbsolutePath() + "/CustomThingsResourcepack"),
            "Custom Things Resource Pack",
            CustomThings.MODID);
    ...
    private static void addIcons(ResourcePackAssembler assembler) {
        initialize("icons");
        for (File f : new File(baseDir.getAbsolutePath() + "/icons").listFiles(
            (FileFilter) FileFilterUtils
                .or((IOFileFilter) EnderFileUtils.pngFilter, FileFilterUtils.suffixFileFilter(".mcmeta")))) {
            assembler.addIcon(f);
        }
    }
    ...
    private static void addCustoms(ResourcePackAssembler assembler) {
        ...
        for (File f : new File(baseDir.getAbsolutePath() + "/recordMusic")
            .listFiles((FileFilter) FileFilterUtils.suffixFileFilter(".ogg"))) {
            ...
            root.add("records." + CustomThings.MODID + "." + getSimpleName(f), event); // event name (same as name sent
                                                                                       // to ItemCustomRecord)
        }
        assembler.addCustomFile(
            "assets/minecraft",
            EnderFileUtils.writeToFile(baseDir.getAbsolutePath() + "/recordMusic/sounds.json", gson.toJson(root)));
    }
